...
Some care is taken to not emit completely absurd code. Immediates may be encoded as the result of an an expression. So, with a small change to the compiler it would be possible to use immediate-arithmetic instructions, for example: Instead of the above we could emit
```s
li a0, 1
add a0, a0, a1
```
...
The focus for this semester was to get a small core language to compile. This builds a solid foundation, which can be iterated on next semester.

The most notable omissions are in the typechecker. Currently it doesn't handle any kind of polymorphism, and only has basic support for numbers.

The compiler is well partitioned, each step outlined in section III is clearly seperated. An AST is generated by the parser, passed through the typechecker then transformed into an assembler file. Each module has minimal dependencies on any other.

== Syntax Progress

Type expressions support a subset of the possible scalars: `unit` and `bool`, and integer types. 
Support for number (i.e. integer) ranges (`0..10`) has been established, although negative numbers are not included.
Container types, arrays and structures have been fully implemented in syntax.
Named references to other types have also been implemented, which may include type parameters.
The most notable omission in the current type expression grammar is that there is no union operator.

Most of the top level definitions have been implemented: `type`, `func`, and `extern func`.
The most notable omission is proper module syntax.
Each of the these definitions also has support for type parameters.

Expression syntax is lacking. There is only support for integer comparison operators, basic arithmetic operations, and assignment. Bindings are supported via `let`, and may be referenced by name. Container type access is also supported.. Basic branching via `if` and `while` (`if`, `while`, and `let`, all have a value and type, allowing them to be used in expressions).
